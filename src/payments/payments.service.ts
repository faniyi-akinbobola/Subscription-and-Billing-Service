...
export class PaymentsService {
  ...
  async confirmPaymentIntent(
    ...
    try {
      ...
      this.logger.info(`Payment intent confirmed: ${paymentIntent.id}`);
      return paymentIntent;
    } catch (error) {
  ...
  private async handlePaymentIntentSucceeded(
    ...
    this.logger.info(`Payment successful for: ${paymentIntent.id}`);
    ...
    try {
      ...
      if (existingPayment) {
        ...
        this.logger.info(`Updated payment record: ${existingPayment.id}`);
      } else {
        ...
        this.logger.info(`Created payment record: ${payment.id}`);
      }
    } catch (error) {
      this.logger.error(
  ...
  private async handlePaymentIntentFailed(
    ...
    this.logger.info(`Payment failed for: ${paymentIntent.id}`);
    ...
    try {
      ...
      if (existingPayment) {
        ...
        await this.paymentRepository.save(payment);
        this.logger.info(`Created failed payment record: ${payment.id}`);
      }
  ...
  private async handleInvoicePaymentSucceeded(
    ...
    this.logger.info(`Invoice payment successful: ${invoice.id}`);
  ...
  private async handleSubscriptionTrialWillEnd(
    ...
    this.logger.info(`Subscription trial ending soon: ${subscription.id}`);
